...
public class IssueCompaction {
	public static final Log LOG = LogFactory.getLog(HIndexConstantsAndUtils.class);
    ...
    static void fakedLoadData(byte[] dataTableName) throws IOException{
        ...
        p.addColumn(columnFamily, Bytes.toBytes(indexedColumnName), 101, Bytes.toBytes("v1"));
        htable.put(p);

        //put value2
        p = new Put(rowKey);
        p.addColumn(columnFamily, Bytes.toBytes(indexedColumnName), 102, Bytes.toBytes("v2"));
        htable.put(p);
    }
...
//            admin.flush(tn);
    ...
    public static void fakedTest(byte[] dataTableName){
        try{
            ...
            g.setMaxVersions();
            res = htable.get(g);
            List<Cell> rl = res.getColumnCells(columnFamily, Bytes.toBytes(indexedColumnName));
